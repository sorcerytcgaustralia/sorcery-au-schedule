...
- **The map of the realm is the identity and the place selector.** An ink
  map of Australia on parchment in the manner of Tolkien's maps: the real
  coast from Natural Earth (`src/lib/australia.ts`, generated by
  `scripts/build-map.py`), hatching on the sea side of the coast, the
  ranges as pen strokes, the seas named, a compass and a cartouche. The
  seven cities are real buttons over it (`src/components/Chart.tsx`,
  positions from `src/lib/chart.ts`). Selecting a city sets the place for
  the whole page; the live line, the register and the places follow.
...
src/components/Realm.tsx, Chart.tsx   the statement, the live line, the Discord note, the map
...
src/lib/chart.ts, australia.ts        projection, city points, label sides; generated coast geometry
